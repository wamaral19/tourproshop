...
function safeRedirectTarget(raw: string | null | undefined): string {
  if (!raw) return APPAREL_DEFAULT_TARGET;
...
export async function GET(
  ...
  let target = APPAREL_DEFAULT_TARGET;
  let isKnown = false;
  if (validSlug && db) {
    try {
      const link = await lookupApparelLink(db, slug);
      if (link) {
        isKnown = true;
        target = safeRedirectTarget(link.target_url);
      }
    } catch (err) {
      console.error("[apparel-outreach] lookup failed", err);
    }
  }
  ...
  const clickPromise = (async () => {
    ...
    try {
      ...
      console.error("[apparel-outreach] click insert failed", err);
    }
  })();
  ...
  const cookie = [
    `${APPAREL_COOKIE_NAME}=${encodeURIComponent(cookieValue)}`,
