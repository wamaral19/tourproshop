...
function safeRedirectTarget(raw: string | null | undefined): string {
  if (!raw) return SPONSOR_DEFAULT_TARGET;
...
export async function GET(
  ...
  let target = SPONSOR_DEFAULT_TARGET;
  let isKnown = false;
  if (validSlug && db) {
    try {
      const link = await lookupSponsorLink(db, slug);
      if (link) {
        isKnown = true;
        target = safeRedirectTarget(link.target_url);
      }
    } catch (err) {
      console.error("[sponsor-outreach] lookup failed", err);
    }
  }
  ...
  const clickPromise = (async () => {
    ...
    try {
      ...
      console.error("[sponsor-outreach] click insert failed", err);
    }
  })();
  ...
  const cookie = [
    `${SPONSOR_COOKIE_NAME}=${encodeURIComponent(cookieValue)}`,
